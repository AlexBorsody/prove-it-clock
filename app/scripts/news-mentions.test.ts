...
const item = (url: string, date: string, source = 'Example') => `<item><title>Bitcoin &amp; useful - ${source}</title><link>${url}</link><pubDate>${date}</pubDate><source url="https://example.com">${source}</source></item>`;
...
test('counts real items, decodes text, deduplicates, excludes invalid and out of window records', () => {
 ...
 assert.equal(feed.articles[0].title, 'Bitcoin & useful');
...
test('excludes feed matches from unrelated page navigation', () => {
 const feed = parseNewsFeed(wrap(item('https://example.com/unrelated', now.toISOString())), 'bat', now);
