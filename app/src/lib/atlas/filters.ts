...
export interface AtlasFilters { projects: string[]; category: CategoryId | ''; states: AtlasState[]; q: string; promise: string | null }
...
export function matchesAtlas(node: AtlasNode, filters: AtlasFilters): boolean {
  ...
  return (!filters.projects.length||filters.projects.includes(node.projectSlug)) &&
    ...
    (!filters.category||(node.primaryCategory??'unclassified')===filters.category||node.secondaryCategories.includes(filters.category)) &&
