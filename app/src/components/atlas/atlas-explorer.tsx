...
export default function AtlasExplorer({ data }: { data:AtlasDataset }) {
  const params=useSearchParams(); const query=params.toString();
  const resolved=useMemo(()=>revealSelection(parseAtlasQuery(new URLSearchParams(query),data.nodes),data.nodes),[query,data.nodes]);
  ...
  function choose(id:string|null) {
    ...
    if(!id) requestAnimationFrame(()=>{ if(lastFocus.current?.isConnected) lastFocus.current.focus({preventScroll:true});else document.getElementById('atlas-list-toggle')?.focus({preventScroll:true}); });
  }
  ...
  return <div className={styles.atlas}>
    <header className={styles.header} {...searchMeta({id:'atlas-overview',title:'Promise Atlas',kind:'Atlas',keywords:'promise evidence categories subject map'})}>
      <span className={styles.eyebrow}>PROVE VALUE / THE PROMISE LEDGER</span>
    ...
    <div className={styles.resultBar}><span role="status">{matches.length} of {data.nodes.length} promises</span><button type="button" onClick={()=>filter(EMPTY_FILTERS)}>Reset filters</button><a href="#atlas-list-toggle">Browse as a list ↓</a></div>
    ...
    <div className={`${styles.workspace}${selected?' '+styles.withSelection:''}`}>
      ...
      {selected?<aside ref={panel} tabIndex={-1} className={styles.details} aria-labelledby="atlas-detail-title" onKeyDown={e=>{if(e.key==='Escape')choose(null);}}><AtlasDetails node={selected} close={()=>choose(null)}/></aside>:<div className={styles.emptyDetails}><span aria-hidden="true">◎</span><h2>Start with a promise</h2><p>Select a point to inspect its claim, delivery test and evidence.</p><p className={styles.muted}>Different subjects. The same question: did they deliver?</p></div>}
    </div>
    ...
    <details className={styles.list}><summary id="atlas-list-toggle">Promise list <span>{matches.length} {matches.length===1?'match':'matches'}</span></summary>
      {!matches.length?<p>No promises match these filters.</p>:<ul>{matches.map(n=><li key={n.id} {...searchMeta({id:`atlas-promise-${encodeURIComponent(n.id)}`,title:`${n.projectName}: ${n.claimText}`,kind:'Promise',project:n.projectSlug,keywords:`${n.symbol} ${categoryLabel(n.primaryCategory)} ${STATE_LABELS[n.state]}`})}>
