 ...
 *    the snapshot date (2026-09-19), when the seed was snapshotted.
...
const HERE = dirname(fileURLToPath(import.meta.url)); // db/seed/
...
function ins(table: string, columns: string[], values: string): void {
  stmts.push({ table, sql: `INSERT INTO ${table} (${columns.join(", ")}) VALUES (${values});` });
}
